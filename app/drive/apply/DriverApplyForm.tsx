...
export default function DriverApplyForm() {
  const [step, setStep] = useState<"phone" | "otp" | "profile" | "done">("phone");
  ...
  async function sendOtp() {
    ...
    try {
      ...
      setStep("otp");
      setMessage("OTP sent. For development, use 1234.");
    } catch (error: any) {
    ...
    <section className="mx-auto max-w-3xl rounded-[2rem] border border-white/10 bg-white/[0.06] p-8">
      ...
      {step === "otp" && (
        <div className="mt-8 space-y-4">
          ...
          <Button onClick={verifyOtp} loading={loading}>Verify OTP</Button>
        </div>
      )}
      ...
      {step === "profile" && (
        <div className="mt-8 grid gap-4">
          ...
          <Input label="Vehicle Type" value={form.vehicleType} onChange={(v) => setForm({ ...form, vehicleType: v })} />
          <Input label="Vehicle Plate" value={form.vehiclePlate} onChange={(v) => setForm({ ...form, vehiclePlate: v })} />
          <Input label="Vehicle Color" value={form.vehicleColor} onChange={(v) => setForm({ ...form, vehicleColor: v })} />

          <Button onClick={submitDriverApplication} loading={loading}>
            Submit Application
          </Button>
        </div>
      )}
      ...
      {step === "done" && (
        <div className="mt-8 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-5">
          ...
          <p className="mt-2 text-white/70">
            Your driver profile has been submitted to CRUUZ Operations for review.
          </p>
        </div>
      )}
